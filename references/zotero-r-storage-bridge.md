...
2. **Scan only the returned paths** with the skill's own searcher — never the storage root:
   ...
   ```
   python "${CLAUDE_PLUGIN_ROOT:-$(pwd)}/skills/journalresearch/scripts/journalresearch_pdfara.py" \
...
4. In the output, use `Source: Uploaded PDF`; state the "Zotero library"
   ...
   `skills/journalresearch/scripts/journalresearch_pubmedara.py --query`.
