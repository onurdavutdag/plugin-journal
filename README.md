...
|---|---|
| **Python 3** + `python-docx` | Every `.docx` operation — formatting (`journalstyle_apply_profile.py`) and the citation/bibliography render (`zotero_docxatifbas.py`). |
...
of its own, and each path degrades to a non-MCP fallback — PubMed to `journalresearch_pubmed_eutils.py`, the
