...
0. **Resolve the workspace (before everything else).** The plugin runs every job through the
   **folder containing the source `.docx`** — or, when the source sits in the plugin checkout's
   ...
   `ciktilar/` or `output/`.
   **Output layout (1.22.0).** The JSON also carries `stamp` (`YYYYMMDD HHMM`, this job's start)
   and `output_layout: "ext-subdir"`: every file this job produces lives in an **extension
   subfolder** of `<outputs_dir>` and ends in that stamp — `docx/<manuscript>_<slug> <stamp>.docx`,
   `pdf/… <stamp>.pdf`, `md/… <stamp>.md`. Never compose such a path by hand; ask the plugin-root
   ...
   `_v2` or an earlier stamp are stripped from `--ad` — the stamp is the version. The resolver
   moves this manuscript's earlier versions in that subfolder to `<ext>/yedekler/` (listed in
   `yedeklenen`; other documents stay — 1.24.0); when the source `.docx` itself sits in that subfolder, add `--kaynak "<source>"`.
...
5. **Verify and report.** **Start the report with the provenance block** (see "Report provenance"). After applying, run `${CLAUDE_PLUGIN_ROOT:-$(pwd)}/skills/journalstyle/scripts/journalstyle_docxyapicikar.py` again and check against the profile requirements (word limit, required sections).
   - **Word check (when Microsoft Word is installed):** `python "${CLAUDE_PLUGIN_ROOT:-$(pwd)}/scripts/office_kopru.py" check "<the formatted docx>" --outputs-root "<outputs_dir>"` opens the output in real Word, invisibly and read-only, and returns `repair_prompt`, `protected_view`, `compat_mode`, `page_count`, `page_size_in` and `fonts_missing` — the profile's font must be installed here, or the PDF the journal sees will carry a substitute. Put those five facts in the compliance report. If the user wants a submission PDF: `… office_kopru.py pdf "<that docx>" --out "<pdf path>" --outputs-root "<outputs_dir>"`, the `--out` value from `cikti_yolcoz.py … --uzanti pdf --ek "_<slug>" --damga "<stamp>"` → `pdf/<manuscript>_<slug> <stamp>.pdf` (Word's own export, print quality). Exit 2 `no_office` → write "Word not on this machine — verification stays manual" and continue; `modal_detected` → show the `_modal-N.png` it names and the dialog text, never retry with `--quiet-alerts` silently. The check reads; it never writes the docx. Verifying and fixing reference/citation **format is `journal-s-zotero`'s job** — here only add a note "is the citation style compatible with the journal, and if not, direct to `journal-s-zotero`". Give the user a short "compliance report": list what was fixed automatically, what needs a manual check (e.g. figure/table placement, copyright permissions, `journal-s-zotero` for citation/bibliography). Also, in the report, **compare** the source manuscript's de-facto table/figure/reference count and style with the journal's typical values (`<slug>.yayinstili.json`) (e.g. "this journal has a median of 3 tables, the draft has 7 tables — simplification could be considered"; "the journal places the figure caption below the visual, the draft has it above").

6. **Multi-journal scenario.** If the same manuscript is prepared for multiple journals, produce separate outputs as `<outputs_dir>/docx/<manuscript-name>_<journal-slug> <stamp>.docx` (one resolver call per journal, same stamp), starting from a clean copy of the source each time. Each journal shares its own `<slug>/` subfolder (yayinstili, authorguidelines) and profiles within the same workspace. State any non-shared requirements (e.g. word limit difference) separately in the report.
...
- Always back up the original file before touching the docx (`<outputs_dir>/docx/<name>_original_backup <stamp>.docx` — beside the output, never beside the source).
...
- `output/` receives every file this skill produces (formatted docx, backup, reports) — in
  extension subfolders with the job stamp (`output/docx/… <stamp>.docx`, `output/md/… <stamp>.md`),
  the layout `scripts/cikti_yolcoz.py` owns.
...
- **`${CLAUDE_PLUGIN_ROOT:-$(pwd)}/scripts/cikti_yolcoz.py`** (plugin root, owned by no skill) — resolves every output path (`<outputs_dir>/<ext>/<name> <stamp>.<ext>`); the only place an output name is composed.
- **`${CLAUDE_PLUGIN_ROOT:-$(pwd)}/scripts/office_kopru.py`** (plugin root, owned by no skill) — opens the formatted docx in real Word for Step 5's check (`check`) and the submission PDF (`pdf`); PowerShell COM, optional, exit 2 `no_office` when Word is absent.
