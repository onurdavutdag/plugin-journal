...
| `journal-s-zotero` (`zotero_docxatifbas.py`) | agent (Read, Glob, Grep, Bash) | when the section is written into a docx | in-text citations + bibliography |
...
- `SKILL.md` — the flow.
- `references/journalwriter-s-danisman-r-bilgi.md` — distilled manuscript-writing knowledge (the advisor's source).
- `references/journalwriter-s-danisman-r-guidelines/` — ARRIVE · CARE · CONSORT · PRISMA · STARD · STROBE,
  item level. Also **reused read-only by `journalpeerreview`**.
