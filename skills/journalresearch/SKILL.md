...
```
python "${CLAUDE_PLUGIN_ROOT:-$(pwd)}/skills/journalresearch/scripts/journalresearch_pdfara.py" --dir <workspace-or-project-dir> --terms "keyword" "phrase" ...
```
...
`scripts/journalresearch_pubmedara.py`, which queries the public NCBI E-utilities API with **no auth**:

```
PLUGIN="${CLAUDE_PLUGIN_ROOT:-$(pwd)}"
python "$PLUGIN/skills/journalresearch/scripts/journalresearch_pubmedara.py" --query "clear question / keywords" --retmax 5
python "$PLUGIN/skills/journalresearch/scripts/journalresearch_pubmedara.py" --pmid 34567890            # verify one record
python "$PLUGIN/skills/journalresearch/scripts/journalresearch_pubmedara.py" --doi 10.1001/jama.2019.4783   # resolve a DOI
```
