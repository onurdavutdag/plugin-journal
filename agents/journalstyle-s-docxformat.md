...
**Every change to the document goes through `journalstyle_docxbicimuygula.py`** — you hold no `Write`/`Edit` tool, and
...
- **Missing `required_sections`:** report them. Placeholder headings are added only with the user's
  approval and only through `journalstyle_docxbicimuygula.py --add-sections` — never silently, never by hand.
