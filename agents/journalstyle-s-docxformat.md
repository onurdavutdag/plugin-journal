...
**Every change to the document goes through `apply_profile.py`** — you hold no `Write`/`Edit` tool, and
...
5. **Missing required sections — ask first, then let the script add them.** Name the missing sections to
   ...
   with the flag:
   `${CLAUDE_PLUGIN_ROOT:-$(pwd)}/skills/journalstyle/scripts/apply_profile.py <input> <profile.json> <output> --add-sections`
...
- **Missing `required_sections`:** report them. Placeholder headings are added only with the user's
  approval and only through `apply_profile.py --add-sections` — never silently, never by hand.
